...
def create_geometry():
    ...
    geo.point([0, 0], ID=p_id); p1=p_id; p_id+=1
    geo.point([0, 0.046], ID=p_id); p2=p_id; p_id+=1
    geo.point([0.01, 0.046], ID=p_id); p3=p_id; p_id+=1
    geo.point([0.01, 0], ID=p_id); p4=p_id; p_id+=1
    
    # Lines for the rectangle
    l1=l_id; geo.line([p1, p2], ID=l1, marker=NO_BOUNDARY_CONDITION); l_id+=1
    l2=l_id; geo.line([p2, p3], ID=l2, marker=BOUNDARY_CONDITION_CONVECTION); l_id+=1
    l3=l_id; geo.line([p3, p4], ID=l3, marker=NO_BOUNDARY_CONDITION); l_id+=1
    l4=l_id; geo.line([p4, p1], ID=l4, marker=BOUNDARY_CONDITION_QOUT); l_id+=1
    ...
    for c_coord in centers:
        ...
        cp = p_id; geo.point([x, y], ID=cp, el_size=c_el_size); p_id+=1
        pr = p_id; geo.point([x+r, y], ID=pr, el_size=c_el_size); p_id+=1
        pt = p_id; geo.point([x, y+r], ID=pt, el_size=c_el_size); p_id+=1
        pl = p_id; geo.point([x-r, y], ID=pl, el_size=c_el_size); p_id+=1
        pb = p_id; geo.point([x, y-r], ID=pb, el_size=c_el_size); p_id+=1
        
        # Create lines for the circle and assign boundary conditions
        c1=l_id; geo.circle([pr, cp, pt], ID=c1, marker=BOUNDARY_CONDITION_CONVECTION); l_id+=1
        c2=l_id; geo.circle([pt, cp, pl], ID=c2, marker=BOUNDARY_CONDITION_CONVECTION); l_id+=1
        c3=l_id; geo.circle([pl, cp, pb], ID=c3, marker=BOUNDARY_CONDITION_CONVECTION); l_id+=1
        c4=l_id; geo.circle([pb, cp, pr], ID=c4, marker=BOUNDARY_CONDITION_CONVECTION); l_id+=1
    ...
    coords, edof, dofs, bdofs, elementmarkers = cfm.create_mesh(geo, el_type=2, el_size_factor=0.08, dofs_per_node=1)
